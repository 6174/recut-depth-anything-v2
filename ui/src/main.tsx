...
import { AlertTriangle, Check, Download, FolderOpen, Image, LoaderCircle, Save, Send, SlidersHorizontal, Upload, Video } from "lucide-react";
...
function App() {
  ...
  const prepare = useCallback(async () => {
    ...
    setFailure("");
    setMessage("正在启动…");
    try { const result = await recut.background.call("depth.prepare") as { job: ShellJob }; setActiveJob({ id: result.job.id, action: "prepare" }); setLogs([]); }
...
function Setup({ busy, failure, logs, message, onPrepare, onAskAgent }: { busy: string | null; failure: string; logs: string[]; message: string; onPrepare: () => void; onAskAgent: () => void }) {
  const started = useRef(false);
  useEffect(() => { if (!started.current) { started.current = true; onPrepare(); } }, [onPrepare]);
  return <main className="setup"><div className="setup-mark"><LoaderCircle className="spin" size={22} /></div><p className="eyebrow">DEPTH ANYTHING</p><h1>正在启动</h1><p>首次使用需要一点时间，完成后即可选择模型并开始转换。</p>{busy === "prepare" && <pre className="job-log" aria-label="准备过程">{logs.length ? logs.join("") : "正在连接…"}</pre>}{failure && <div className="error-box"><strong>暂时无法启动</strong><pre>{failure}</pre><button className="link-button" disabled={busy !== null} onClick={onAskAgent} type="button"><Send size={14} />交给右侧 Codex 处理</button></div>}<button className="secondary-button" disabled={busy !== null} onClick={onPrepare} type="button">{busy === "prepare" ? <LoaderCircle className="spin" size={16} /> : <Download size={16} />}{busy === "prepare" ? "正在启动…" : "重新尝试"}</button><p className="status" role="status">{message}</p></main>;
}
