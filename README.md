...
1. 在 **Apps** 中从 [recut-depth-anything-v2](https://github.com/6174/recut-depth-anything-v2) 安装并打开“Depth Anything”。首次进入会自动准备 manifest 声明的 Python 与 FFmpeg，再创建隔离 venv、安装锁定依赖与执行 App bootstrap；用户无需配置 Python、Homebrew 或 PATH。
...
`manifest.json` 的 `runtime.python` 是唯一的环境声明：平台自动取得声明的 Python 与 FFmpeg，创建 venv、按 `python/requirements.lock` 安装 PyTorch、TorchVision、OpenCV 和 timm；`bootstrap.py` 是跨平台的兜底脚本，本 App 用它下载 [Depth Anything V2](https://github.com/DepthAnything/Depth-Anything-V2) 官方代码。视频逐帧推理先写入中间文件，再经 FFmpeg 转为浏览器可播放的 H.264/yuv420p MP4；安装、下载和推理均作为可取消任务运行，模型下载从开始、每 5% 到完成均输出 stdout，视频逐帧进度和错误同样显示在界面和项目事件流中，错误可直接交给右侧 Codex 处理。
...
bootstrap.py            跨平台的官方代码准备兜底脚本；不拥有 venv 或 pip 生命周期
